...
if [[ ! -f "$SELECTED_VIDEO" ]]; then
  ...
  case "$DE" in
    kde)
      ...
      install_pkg qt5-tools qttools5-dev-tools
      if ! command -v qdbus &>/dev/null; then
        echo "❌ qdbus non disponible. Impossible d'appliquer le fond animé KDE automatiquement."
      else
      ...
      ;;
    gnome)
      echo "🎥 Intégration fond animé dans GNOME (fallback statique)..."
